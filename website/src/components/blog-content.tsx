...
import { articles, type BlogArticle } from "@/app/blog/blog-data";
...
function localized(article: BlogArticle, t: (key: string) => string, field: "title" | "excerpt" | "category"): string {
  const key = `blog.articles.${article.slug}.${field}`;
  const translated = t(key);
  return translated !== key ? translated : article[field];
}
          ...
          {articles.map((article) => (
            ...
            >
              ...
              <h2 className="text-lg font-semibold group-hover:text-primary transition-colors line-clamp-2">
                {localized(article, t, "title")}
              </h2>
              <p className="mt-2 text-sm text-muted-foreground line-clamp-3">{localized(article, t, "excerpt")}</p>
